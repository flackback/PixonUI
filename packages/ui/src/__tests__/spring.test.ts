...
describe('generateSpringKeyframes', () => {
  test('generates valid keyframes starting at 0 and ending at 1', () => {
    const { keyframes, duration } = generateSpringKeyframes({
      stiffness: 100,
      damping: 10,
    });
  ...
  test('respects high damping (settles faster when critically damped vs underdamped)', () => {
    const underdamped = generateSpringKeyframes({ stiffness: 100, damping: 2 });
    const criticallyDamped = generateSpringKeyframes({ stiffness: 100, damping: 20 });
