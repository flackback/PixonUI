...
export function ChatHeader({ 
  ...
  const handleInfo = onInfoClick || onInfo;
  const displayName = user?.name || group?.name || "Chat";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="relative cursor-pointer" onClick={onAvatarClick || handleInfo}>
          <Avatar src={avatarUrl} alt={displayName} fallback={fallbackChar} />
          {user?.status === 'online' && (
        ...
        <div className="cursor-pointer" onClick={handleInfo}>
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900 dark:text-white leading-none">{displayName}</h3>
          ...
          <p className={cn(
            ...
            {isTyping ? 'typing...' : user?.status === 'online' ? 'Online' : user?.lastSeen ? `Last seen ${user.lastSeen.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}
          </p>
        </div>
      </div>
